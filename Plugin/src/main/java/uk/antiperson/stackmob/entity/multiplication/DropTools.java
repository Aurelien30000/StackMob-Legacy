...
public class DropTools implements IDropTools {
    ...
    private Map<ItemStack, Integer> calculateDrops(int deadAmount, LivingEntity dead, List<ItemStack> entityDrops) {
        Map<ItemStack, Integer> drops = new HashMap<>();
        for (int i = 0; i < deadAmount; i++) {
            for (ItemStack stack : generateLoot(dead, entityDrops)) {
    ...
    public int calculateAmount(int multiplier) {
        return (int) Math.round((0.75 + ThreadLocalRandom.current().nextDouble(1)) * multiplier);
    }
